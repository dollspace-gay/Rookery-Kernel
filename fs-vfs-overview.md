---
title: "Tier-3: fs/vfs/00-overview — VFS core hub"
...
- REQ-2: VFS data-model invariants (cross-ref sub-docs):; One `inode` per file (across all hardlinks); refcount-managed; One `dentry` per name in the dcache (cached path component); One `file` per open(2) call; multiple `file`s can share an `inode`; One `super_block` per mounted filesystem instance; One `vfsmount` per mountpoint in a mount namespace
