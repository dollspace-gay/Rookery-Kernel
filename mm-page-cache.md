---
title: "Tier-3: mm/page-cache — filemap, folio operations, readahead, truncate"
...
- REQ-3: Folio lifecycle (allocate → insert into cache → mark dirty → writeback → invalidate / truncate → free) preserves upstream's per-state invariants:; Newly-allocated folio: not in any cache, refcount = 1; Cache-resident folio: in `i_pages` xarray, refcount ≥ 1; Locked folio: PageLocked set; unique writer; readers may still hold references; Dirty folio: PageDirty set + xarray DIRTY mark + per-mm dirty accounting incremented; Writeback-in-progress: PageWriteback set + xarray TOWRITE/WRITEBACK marks; Invalidated folio: not in cache, but may be still referenced by readers (handled via release_folio path)
