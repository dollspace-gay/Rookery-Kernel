# Tier-5: include/uapi/linux/bpf.h — BPF / eBPF UAPI
...
upstream-paths:
  - include/uapi/linux/bpf.h (~7705 lines)
...
- Whether to expose `BPF_F_VTYPE_BTF_OBJ_FD` and `value_type_btf_obj_fd` semantics fully in this Tier-5, or defer the kernel-defined-type subset to `bpf/struct_ops.md`.
- Whether to publish `enum bpf_perf_event_type` reply shapes here or in a follow-up Tier-5 for the perf-event subsystem.
- Treatment of the `___BPF_FUNC_MAPPER` X-macro: regenerate the helper-ID enum mechanically from a single source-of-truth at build time, or hand-maintain a parallel Rust enum? The former wins on ABI stability; the latter wins on grep-ability. Tentative answer: generate.
- `BPF_PROG_TYPE_NETFILTER` vs `BPF_LINK_TYPE_NETFILTER` overlap with the in-kernel nftables surface — exact share-of-state semantics need confirmation against `net/netfilter/nf_bpf_link.c`.
- `BPF_MAP_TYPE_ARENA` and `BPF_MAP_TYPE_INSN_ARRAY` are recent additions; their long-term ABI guarantees vs being declared experimental need a policy call.
