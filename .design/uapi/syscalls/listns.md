# Tier-5 syscall: listns(2) — forward-looking (ENOSYS in 7.1.0-rc2)
...
upstream-paths:
  - kernel/nsproxy.c (proposed: SYSCALL_DEFINE3(listns, ...))
  - fs/proc/proc_namespace.c (existing /proc/<pid>/ns/* surface — current discovery path)
  - include/uapi/linux/nsfs.h (NS_GET_USERNS, NS_GET_PARENT, NS_GET_NSTYPE)
  - kernel/user_namespace.c (CAP_SYS_PTRACE check on cross-userns)
  - arch/x86/entry/syscalls/syscall_64.tbl (470  common  listns  (reserved, not wired))
-->
