# Tier-5 syscall: fork(2) — syscall 57
...
upstream-paths:
  - kernel/fork.c (sys_fork, SYSCALL_DEFINE0(fork))
  - include/linux/syscalls.h (asmlinkage long sys_fork(void))
  - arch/x86/entry/syscalls/syscall_64.tbl (57  common  fork)
  - arch/arm64 has NO fork — wired to clone via vDSO/libc (generic-sys archs use only clone/clone3)
-->
...
- (none at this Tier-5 level — fork is the simplest of the clone family)
