# Tier-5 syscall: getsockopt(2) — get socket option (syscall 55)
...
upstream-paths:
  - net/socket.c (__sys_getsockopt, sock_getsockopt)
  - net/core/sock.c (sock_getsockopt, do_sock_getsockopt)
...
- Should `SO_PEERPIDFD` (introduced upstream 2024) require CAP_SYS_ADMIN by default under hardened build? (Upstream: no cap; we lean toward CAP_SYS_PTRACE-or-self.)
- BPF post-getsockopt may extend `*optlen` beyond computed; do we cap to caller's max? Upstream: yes, hard cap.
