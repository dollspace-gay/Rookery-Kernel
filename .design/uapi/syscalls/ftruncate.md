# Tier-5 syscall: ftruncate(2) — syscall 77
...
upstream-paths:
  - fs/open.c (SYSCALL_DEFINE2(ftruncate), do_sys_ftruncate, do_truncate)
  - fs/attr.c (notify_change, setattr_should_drop_suidgid)
  - mm/filemap.c (truncate_pagecache, truncate_inode_pages)
  - include/linux/fs.h (struct file, struct iattr, ATTR_SIZE)
  - arch/x86/entry/syscalls/syscall_64.tbl (77  common  ftruncate)
-->
...
- `truncate(2)` / `ftruncate64(2)` / `fallocate(2)` / `memfd_create(2)` /
  `shm_open(3)` (separate Tier-5 docs).
- Implementation code.
