# Tier-5 syscall: truncate(2) — syscall 76
...
upstream-paths:
  - fs/open.c (SYSCALL_DEFINE2(truncate), do_sys_truncate, do_truncate)
  ...
  - include/linux/fs.h (struct iattr, ATTR_SIZE, ATTR_KILL_SUID)
  - arch/x86/entry/syscalls/syscall_64.tbl (76  common  truncate)
-->
...
- `ftruncate(2)` / `truncate64(2)` / `fallocate(2)` / `open(O_TRUNC)`
  (separate Tier-5 docs).
- Implementation code.
