# Tier-5 syscall: fanotify_mark(2) — syscall 301
...
upstream-paths:
  - fs/notify/fanotify/fanotify_user.c (SYSCALL_DEFINE5(fanotify_mark), do_fanotify_mark)
  - fs/notify/fsnotify.c (fsnotify_add_mark, fsnotify_destroy_mark)
  - include/uapi/linux/fanotify.h (FAN_MARK_*, FAN_*_EVENTS)
  - include/linux/fanotify.h (fanotify_mark)
  - arch/x86/entry/syscalls/syscall_64.tbl (301  common  fanotify_mark)
  - Documentation/admin-guide/filesystem-monitoring.rst, man fanotify_mark(2)
-->
...
- Behavior of `FAN_MARK_EVICTABLE` under shrinker pressure when the listener still wants events: currently the next event after eviction silently drops. Hardened policy may require the listener be notified via a synthetic `FAN_MARK_LOST` event so it can re-add. To be defined in `uapi/headers/fanotify.md`.
