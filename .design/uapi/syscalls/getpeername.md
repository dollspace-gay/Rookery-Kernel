# Tier-5 syscall: getpeername(2) — return the peer's address of a connected socket (syscall 52)
...
upstream-paths:
  - net/socket.c (__sys_getpeername, sys_getpeername, move_addr_to_user)
  - net/ipv4/af_inet.c (inet_getname with peer=1)
  - net/ipv6/af_inet6.c (inet6_getname with peer=1)
  - net/unix/af_unix.c (unix_getname with peer=1)
  - net/netlink/af_netlink.c (netlink_getname with peer=1)
-->
...
- For TCP_SYN_SENT state (handshake in flight), should getpeername return the not-yet-confirmed peer address or `-ENOTCONN`? Upstream: returns the address (peer is "set" even if not ack'd). We follow.
- For UDP that `connect()`ed and then `connect(AF_UNSPEC)`'d to disassociate, getpeername returns `-ENOTCONN`. Verified.
