# Tier-5 syscall: setsockopt(2) — set socket option (syscall 54)
...
upstream-paths:
  - net/socket.c (__sys_setsockopt, sock_setsockopt)
  - net/core/sock.c (sock_setsockopt, do_sock_setsockopt)
  ...
  - include/uapi/linux/in.h (IP_* constants)
  - include/uapi/linux/tcp.h (TCP_* constants)
-->
...
- Should `TCP_CONGESTION` lookup be restricted to a CAP_NET_ADMIN whitelist under hardened build? (Upstream: any user can set non-`*_FORCE_*` ones.)
- For BPF-rewritten `optlen`: do we expose the original optlen to LSM, or the rewritten value? Upstream: rewritten.
