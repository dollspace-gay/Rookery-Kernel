# Tier-5 syscall: splice(2) — splice data to/from a pipe (syscall 275)
...
upstream-paths:
  - fs/splice.c (__do_sys_splice, do_splice, splice_pipe_to_pipe, do_splice_from, do_splice_to)
  - fs/splice.c (generic_splice_sendpage, generic_file_splice_read, iter_file_splice_write)
  - fs/pipe.c (pipe_lock, pipe_unlock)
  - include/uapi/linux/fs.h (SPLICE_F_MOVE, SPLICE_F_NONBLOCK, SPLICE_F_MORE, SPLICE_F_GIFT)
-->
...
- Should `SPLICE_F_GIFT` be denied for all non-vmsplice paths (currently silently ignored)? Upstream: ignored. We follow.
- `splice` from `/dev/zero` to a socket as a cheap zero-fill — allowed today, should hardened policy gate it (anti-DoS)? Upstream: allowed; grsec hardening may rate-limit.
