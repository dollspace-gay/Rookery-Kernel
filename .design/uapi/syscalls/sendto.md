# Tier-5 syscall: sendto(2) — send a message on a socket with explicit destination (syscall 44)
...
upstream-paths:
  - net/socket.c (__sys_sendto, sys_sendto, sys_send, move_addr_to_kernel)
  - net/socket.c (sock_sendmsg, sock_sendmsg_nosec)
  - net/ipv4/udp.c (udp_sendmsg)
  - net/ipv4/tcp.c (tcp_sendmsg)
  - net/unix/af_unix.c (unix_dgram_sendmsg, unix_stream_sendmsg)
...
- For `MSG_ZEROCOPY` with `sendto`: completion path via `MSG_ERRQUEUE`. Document interaction in `recvmsg.md`. We follow upstream.
- For `MSG_FASTOPEN` with sendto on a CLOSE-state TCP socket: kernel implicitly connect()s. Document.
