# Tier-5 syscall: sendmsg(2) — send a message with ancillary data (syscall 46)
...
upstream-paths:
  - net/socket.c (__sys_sendmsg, ___sys_sendmsg, sendmsg_copy_msghdr)
  - net/compat.c (get_compat_msghdr) [compat path]
  - net/core/scm.c (scm_send, __scm_send)
  - include/linux/socket.h (struct msghdr, struct cmsghdr, struct user_msghdr)
  - include/uapi/linux/socket.h (CMSG_* macros, SCM_* control types)
-->
...
- Should we lower `SCM_MAX_FD` from 253 to a more conservative value (e.g. 32) for hardened builds? Upstream grsec: no.
- For `MSG_ZEROCOPY` with SCM_RIGHTS combined: behavior is unspecified; we follow upstream rejection if both are set.
