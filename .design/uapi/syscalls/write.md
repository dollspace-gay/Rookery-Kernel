...
upstream-paths:
  ...
  - fs/read_write.c (`SYSCALL_DEFINE3(write, ...)`, `ksys_write`, `vfs_write`, `rw_verify_area`, `new_sync_write`, `file_ppos`)
  - include/linux/fs.h (`struct file`, `FMODE_WRITE`, `FMODE_CAN_WRITE`, `FMODE_STREAM`, `FMODE_PWRITE`, `i_size_write`)
  - include/linux/syscalls.h (`asmlinkage long sys_write(unsigned int, const char __user *, size_t)`)
-->
