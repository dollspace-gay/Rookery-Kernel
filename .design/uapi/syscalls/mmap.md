...
upstream-paths:
  - arch/x86/entry/syscalls/syscall_64.tbl (`9  common  mmap  sys_mmap`)
  - arch/x86/kernel/sys_x86_64.c (`SYSCALL_DEFINE6(mmap, ...)` arch wrapper → ksys_mmap_pgoff with byte-to-page-shift)
  - mm/mmap.c (`SYSCALL_DEFINE6(mmap_pgoff, ...)`, `ksys_mmap_pgoff`, `do_mmap`, `mmap_region`, `get_unmapped_area`)
  - mm/util.c (`vm_mmap_pgoff`)
  - include/uapi/asm-generic/mman-common.h (`PROT_*`, `MAP_FIXED`, `MAP_ANONYMOUS`, `MAP_POPULATE`, `MAP_NONBLOCK`, `MAP_STACK`, `MAP_HUGETLB`, `MAP_SYNC`, `MAP_FIXED_NOREPLACE`, `MAP_UNINITIALIZED`)
  - include/uapi/asm-generic/mman.h (`MAP_GROWSDOWN`, `MAP_DENYWRITE`, `MAP_EXECUTABLE`, `MAP_LOCKED`, `MAP_NORESERVE`)
-->
