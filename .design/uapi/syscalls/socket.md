# Tier-5 syscall: socket(2) — create a socket endpoint (syscall 41)
...
upstream-paths:
  - net/socket.c (sys_socket / __sys_socket / sock_create / sock_map_fd)
  - include/linux/net.h (SOCK_* enum, SOCK_CLOEXEC, SOCK_NONBLOCK, SOCK_TYPE_MASK)
  - include/uapi/linux/socket.h (AF_* / PF_* constants)
  - include/linux/socket.h (struct socket, struct proto_ops)
-->
...
- Should `AF_VSOCK` host-side require `CAP_NET_ADMIN` in addition to vsock-specific permissions? (Tracking upstream behavior.)
- `AF_QIPCRTR` (Qualcomm IPC router) — out of scope for Rookery v1?
