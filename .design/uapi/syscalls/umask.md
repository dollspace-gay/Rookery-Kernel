# Tier-5 syscall: umask(2) — syscall 95
...
upstream-paths:
  - kernel/sys.c (SYSCALL_DEFINE1(umask), sys_umask)
  - include/linux/fs_struct.h (struct fs_struct, umask field)
  - fs/namei.c (mode masking on open(O_CREAT), mknod, mkdir)
  - arch/x86/entry/syscalls/syscall_64.tbl (95  common  umask)
-->
...
- File-creation syscalls (`open`, `mkdir`, `mknod`, `mkfifo`, `bind`) —
  each separate Tier-5.
