# Tier-5 syscall: fanotify_init(2) — syscall 300
...
upstream-paths:
  - fs/notify/fanotify/fanotify_user.c (SYSCALL_DEFINE2(fanotify_init), fanotify_alloc_group)
  - fs/notify/fanotify/fanotify.c (fanotify_handle_event)
  - include/uapi/linux/fanotify.h (FAN_CLASS_*, FAN_CLOEXEC, FAN_NONBLOCK, FAN_REPORT_*, FAN_UNLIMITED_*)
  - include/linux/fanotify.h (FAN_GROUP_FLAG, fanotify_event)
  - arch/x86/entry/syscalls/syscall_64.tbl (300  common  fanotify_init)
  - Documentation/admin-guide/filesystem-monitoring.rst, man fanotify(7), fanotify_init(2)
-->
...
- Behavior on group close with outstanding permission events: legacy kernel returns `FAN_ALLOW` (so the producer is unstuck); hardened policy may prefer `FAN_DENY` to fail-closed. Selectable via sysctl `fs.fanotify.close_policy` (`allow` | `deny`, default `allow` for compat).
