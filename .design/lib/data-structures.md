# Tier-3: lib/data-structures — kernel data structure library
...
- [ ] AC-10: `make CLIPPY=1 ROOKERY_LINTS=1` over the data-structures crates triggers no `bare_arithmetic_on_size` warnings; bitmap word/bit math, kfifo wraparound, and list_sort recursion-depth all use explicit `checked_*` / `wrapping_*` / `saturating_*`. (covers REQ-11)
