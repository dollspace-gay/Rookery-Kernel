# Tier-3: net/ipv4/tcp_bbr.c — BBR (Bottleneck Bandwidth and Round-trip propagation time) congestion control
...
upstream-paths:
  - net/ipv4/tcp_bbr.c (~1200 lines)
  - include/net/tcp.h (struct tcp_congestion_ops, struct rate_sample)
  - include/net/inet_connection_sock.h (inet_csk_ca)
  - include/linux/win_minmax.h (struct minmax for bw filter)
  - include/uapi/linux/inet_diag.h (INET_DIAG_BBRINFO)
-->
...
- Whether Rookery exposes BBR as a struct_ops BPF program (kfunc registration via `register_btf_kfunc_id_set`) at this Tier-3 or defers to a separate BPF-struct-ops design.
- Confirm `struct rate_sample` layout in Rookery matches upstream `include/net/tcp.h` (covered in `tcp-cong.md` Tier-3).
- Compile-time vs runtime configurability of `bbr_min_rtt_win_sec`, `bbr_probe_rtt_mode_ms` (upstream has no sysctl; some forks expose them).
