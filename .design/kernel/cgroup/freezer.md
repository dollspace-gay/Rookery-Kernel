# Tier-3: kernel/cgroup/freezer.c — cgroup v2 freezer (cgroup.freeze knob, recursive propagation, per-task TIF_FREEZE state machine)
...
upstream-paths:
  - kernel/cgroup/freezer.c (~326 lines)
  - kernel/cgroup/cgroup.c (cgroup.freeze knob registration, cgroup_freezer field)
  - kernel/cgroup/cgroup-internal.h
  - include/linux/cgroup.h (CGRP_FREEZE / CGRP_FROZEN flags; struct cgroup_freezer)
  - include/linux/sched/jobctl.h (JOBCTL_TRAP_FREEZE)
-->
...
- Realtime / SCHED_DEADLINE tasks in a frozen cgroup: the trap occurs at next signal-handling boundary; bounded latency depends on the task's schedule class. Worst-case bound = next `get_signal()` call (usually at syscall return / preempt point). Document expected latency for users with hard-realtime requirements.
- Cgroup v1 has a separate freezer controller (`kernel/cgroup/legacy_freezer.c` if present in baseline; or freezer-v1 attached differently) — out of scope for this Tier-3 but cross-link required.
