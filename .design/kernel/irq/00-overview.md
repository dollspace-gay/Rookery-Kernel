# Tier-2: kernel/irq — IRQ subsystem (irqdesc + irqdomain + irq_chip + msi_domain + manage + softirq + tasklet)
...
(none at Tier-2 — defer to per-Tier-3 once Phase D begins; expected hot questions: tasklet deprecation timeline, default softirq budget, threaded-IRQ default vs `irqsoff` default, NMI watchdog interaction with IRQ migration, RT preempt-RT interaction with `IRQF_NO_THREAD`)
