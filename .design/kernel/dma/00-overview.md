# Tier-2: kernel/dma — DMA-API (mapping + direct + swiotlb + coherent + contiguous + pool + remap + dma-buf consumer)
...
(none at Tier-2 — defer to per-Tier-3 once Phase D begins; expected hot questions: default `swiotlb=` size on confidential-guest [larger needed since every map is bounced], default CMA size for HiDPI laptops with integrated GPU, dma-heap default-mode for `/dev/dma_heap/*`, sync-file refcount cap)
