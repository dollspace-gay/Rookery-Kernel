# Tier-2: drivers/infiniband — RDMA stack (verbs core + RDMA-CM + uverbs/uioctl + IB-MAD/CM + per-HCA hw + sw-rxe/siw + ULPs IPoIB/iSER/SRP/RTRS)
...
(none at Tier-2 — defer to per-Tier-3 once Phase D begins; expected hot questions: ODP enable default for distros [security vs perf trade-off], RDMA cgroup default-on for systemd unit files, vfio-pci-VF binding for ConnectX SR-IOV-VF default behavior, dmabuf-MR-import LSM mediation policy)
