# Tier-2: drivers/input — Input subsystem (input core + evdev/joydev/mousedev/uinput + serio i8042 + force-feedback + per-class keyboards/mice/touchpads/joysticks/touchscreens/tablets/misc)
...
(none at Tier-2 — defer to per-Tier-3 once Phase D begins; expected hot questions: uinput permission for desktop-environment use cases [Wayland virtual-keyboard], BadUSB defense granularity per HID-usage-page, EVIOCREVOKE on user-switch policy, MT-protocol-A vs B preference per-device)
