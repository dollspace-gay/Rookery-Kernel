# Tier-2: drivers/hid — HID subsystem (HID core + USB HID + I2C HID + Bluetooth HID + Intel ISH + AMD SFH + per-vendor + hidraw + hid-bpf)
...
(none at Tier-2 — defer to per-Tier-3 once Phase D begins; expected hot questions: BadUSB authorization granularity per HID-usage-page, uhid `unprivileged-permitted` mode for desktop-environment use, HID-BPF default-policy for distro packagers, Bluetooth LE-keyboard pairing-method default)
