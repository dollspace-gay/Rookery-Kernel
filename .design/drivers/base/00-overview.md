# Tier-2: drivers/base — device-driver model (bus / class / device / driver core)
...
(none at Tier-2 — defer to per-Tier-3 once Phase D begins; expected hot questions: keep `class_create_file` deprecated form vs add new variant only? CONFIG_FW_LOADER_USER_HELPER default — upstream is N for security; Rookery defaults match. Whether to expose `driver_async_probe=*` as default-on for boot speed.)
