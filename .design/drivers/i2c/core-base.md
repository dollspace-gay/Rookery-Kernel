# Tier-3: drivers/i2c/i2c-core-base.c — I2C subsystem core (adapter / client / driver lifecycle, transfer, enumeration)
...
upstream-paths:
  - drivers/i2c/i2c-core-base.c (~2687 lines)
...
- Atomic-xfer fast-path: `master_xfer_atomic` must not sleep (used in shutdown / panic / IPI context); Rust impl must encode `!Send`/no-await constraint or compile-time assert.
- Mux nesting subclass: `i2c_adapter_depth` walks parent chain at-lock-time — should we cache depth on `i2c_register_adapter`? Linux walks each time, which is O(depth) but bounded.
