# Tier-2: drivers/net/ethernet — Ethernet driver vendor-LLD wrapper (Intel + Mellanox + Broadcom + Marvell + Realtek + Aquantia + virtio-net + …)
...
(none at Tier-2 — defer to per-Tier-3 once Phase D begins; expected hot questions: which legacy LLDs to delete entirely vs compile-gate-off, default page-pool size per-LLD, default DIM (interrupt moderation) policy, AF_XDP-zc opt-in vs default-on per LLD)
