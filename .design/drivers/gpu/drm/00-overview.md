# Tier-2: drivers/gpu/drm — Direct Rendering Manager (core + GEM + KMS + atomic + scheduler + TTM + per-vendor amdgpu/i915/xe/nouveau/radeon/v3d/vc4/msm/panfrost/panthor/lima/qxl/virtio-gpu/vmwgfx)
...
(none at Tier-2 — defer to per-Tier-3 once Phase D begins; expected hot questions: amdgpu `dc=auto` vs `dc=on` default for legacy displays, i915 vs xe split policy for transition Intel SKUs, nouveau NVK enable default vs upstream-blob driver coexistence, virtio-gpu venus vs virgl backend default, render-node permission for non-video group users)
