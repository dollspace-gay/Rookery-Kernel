# Subsystem: drivers/ — device drivers (class-tiered + v0 port set)
...
### D1 (2026-05-09): drivers/staging/ out of scope v0
`drivers/staging/` (drivers in transition) gets CONFIG=n by default. No design coverage in v0. Downstream consumers needing a specific staging driver build an out-of-tree FFI shim against the upstream C source.

### D2 (2026-05-09): GPU/DRM FFI in v0; full Rust port deferred to v1+
DRM core + per-GPU drivers (i915, amdgpu, nouveau, msm, etnaviv, panfrost, vc4, virtio-gpu, vmwgfx — ~3M LoC total) stay as upstream C in v0. `drivers/gpu/00-overview.md` Tier 3 documents the FFI boundary. Server/VM users see no impact; desktop users with hardware acceleration use the C-via-FFI path. Rust port is a multi-year v1+ activity.
