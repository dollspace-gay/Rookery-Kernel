# Tier-2: drivers/nvme — NVMe host stack + NVMe-oF target (core + pci + tcp + rdma + fc + multipath + zns + auth + nvmet)
...
(none at Tier-2 — defer to per-Tier-3 once Phase D begins; expected hot questions: NVMe-PCI per-CPU queue policy on systems with > nvme-supported-queue-count CPUs, multipath default `iopolicy` on NUMA-asymmetric NVMe-oF deployments, target loop-transport default for kernel testing, FC transport priority given fc-class shared with SCSI Tier-2)
