# Tier-2: drivers/pci — PCI/PCIe core (probe + capability scan + AER + SR-IOV + ATS/PASID + hotplug + ASPM + MSI/MSI-X)
...
(none at Tier-2 — defer to per-Tier-3 once Phase D begins; expected hot questions: ASPM default policy on laptop vs server SKU, default AER ratelimit, whether to expose `aer_inject` at all in Rookery release builds, default `pcie_bus_safe` vs `tune_off`, CXL.mem coexistence handoff semantics)
