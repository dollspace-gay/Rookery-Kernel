# Tier-2: drivers/media — Media subsystem (V4L2 + media-controller + DVB + CEC + RC + camera ISP + USB-video + PCI-capture)
...
(none at Tier-2 — defer to per-Tier-3 once Phase D begins; expected hot questions: default access-mode for `/dev/video*` (0660 vs 0666 for renderD-style), V4L2 vs libcamera userspace policy (kernel doesn't decide but provides both surfaces), ir-keytable default keymap policy, DVB-CA conditional-access default-disable for distros without smartcard reader)
