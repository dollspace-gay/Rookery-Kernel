# Tier-3: fs/fuse/dev.c — FUSE userspace-fs character device
...
upstream-paths:
  - fs/fuse/dev.c (~2755 lines)
  - fs/fuse/fuse_i.h (struct fuse_conn, fuse_req, fuse_iqueue, fuse_pqueue, fuse_copy_state, FR_* flags)
  - fs/fuse/inode.c (fuse_send_init, FUSE_INIT handshake, FUSE_DEFAULT_MAX_BACKGROUND)
  - include/uapi/linux/fuse.h (FUSE_INIT, FUSE_RELEASE, FUSE_INTERRUPT, FUSE_NOTIFY_*, FUSE_DEV_IOC_*)
...
- Whether `FUSE_DEV_IOC_BACKING_OPEN` / `_BACKING_CLOSE` (FUSE_PASSTHROUGH plumbing) warrant a dedicated Tier-3 (`fuse/passthrough.md`).
