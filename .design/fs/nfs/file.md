# Tier-3: fs/nfs/file.c — NFS regular-file operations
...
upstream-paths:
  - fs/nfs/file.c (~968 lines)
  - fs/nfs/delegation.h
  - fs/nfs/internal.h
  - fs/nfs/pnfs.h
  - fs/nfs/write.c (nfs_wb_*, nfs_commit_inode, nfs_update_folio)
...
- copy_file_range / NFSv4.2 server-side-copy paths: covered in nfs4file.md Tier-3 (not file.c).
- FMODE_CAN_ODIRECT lazily set after nfs_open: any open-after-nfs_open transition where direct-IO must be refused? (None observed; capability is asserted post-revalidate.)
