# Tier-2: fs/ntfs3 — NTFS3 filesystem (Paragon-contributed read+write NTFS, replacing legacy fs/ntfs/ + ntfs-3g FUSE)
...
## Out of Scope
Legacy `fs/ntfs/` (read-only — keep present compile-gated for backward compat); implementation code; 32-bit-only paths.
