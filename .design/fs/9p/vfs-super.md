# Tier-3: fs/9p/vfs_super.c — 9P client filesystem VFS superblock
...
upstream-paths:
  - fs/9p/vfs_super.c (~365 lines)
  - fs/9p/v9fs.c (session init / close / cancel / show_options / parse_param)
  - fs/9p/v9fs.h (struct v9fs_session_info, struct v9fs_context)
  - fs/9p/fid.c (v9fs_fid_add, v9fs_fid_lookup, walk/clunk path-walk)
  - net/9p/client.c (p9_client_create, p9_client_attach, p9_client_destroy)
  - include/net/9p/9p.h (P9_DEBUG_VFS, P9_IOHDRSZ)
  - include/net/9p/client.h (struct p9_fid, struct p9_client)
-->
