# Subsystem: arch/ — architecture support
...
### D1: Arch-abstraction extraction deferred to v1+ first port
The policy decision about which `arch::*` interfaces become `crate::arch::generic::*` is deferred until v1+ port begins. The first v1 port (likely arm64) drives the extraction; the second validates it; the third locks the convention. Per-port lessons land in `arch/00-overview.md` updates as the convention crystallizes. Rationale: a generalized abstraction designed without two concrete arches is the kind of premature architecture that produces technical debt for decades, as upstream Linux's `asm-generic/` history demonstrates.

### D2: UML out of scope for v0
User Mode Linux (`arch/um/`, `arch/x86/um/`) is not in v0 scope. Testing uses QEMU. Re-evaluate in v1+ if UML provides meaningful debugging value. The corresponding upstream subdirectories are simply not reverse-engineered in v0.
