# Subsystem: mm/ — memory management
...
- [ ] AC-14: A grep over the produced kernel `.config` (or runtime `/proc/config.gz`) confirms `CONFIG_SLAB_FREELIST_HARDENED=y` and `CONFIG_SLAB_FREELIST_RANDOM=y` are set by default. A SLUB exploit-fixture targeting freelist mangling (e.g., the standard exploit-edu test) fails identically on Rookery and upstream. (covers REQ-6)
- [ ] AC-15: DAMON selftests under `tools/testing/selftests/damon/` pass with the same pass/fail set as upstream; user-tunable knobs under `/sys/kernel/mm/damon/` accept and reflect identical values. (covers REQ-8)
