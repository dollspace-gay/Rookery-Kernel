...
| Hardening source | grsecurity / PaX patchset at `/home/doll/grsec-6.6.102.patch` is the security-principles reference; cataloged in `.design/references/grsec-pax-notes.md`. **Two-layer split** (locked-in decision 2026-05-09): (a) memory-protection + CFI features are ABSORBED because they are LSM-orthogonal — the locked-in absorption list is **KERNEXEC, UDEREF, USERCOPY, REFCOUNT, RAP/CFI, RANDSTRUCT, AUTOSLAB, PRIVATE_KSTACKS, RANDKSTACK, MEMORY_SANITIZE, SIZE_OVERFLOW, CONSTIFY, LATENT_ENTROPY** plus the other LSM-orthogonal features in the catalog (PAGEEXEC, NOEXEC, MPROTECT, DIRECT_CALL, RANDMMAP, ASLR, DELAY_FREE_ONE_PAGE, CLOSE_KERNEL, CLOSE_USERLAND); SELinux/AppArmor see no behavioral change; (b) GR-RBAC + GRKERNSEC_* policy-enforcement features are NOT absorbed — they conflict with LSM-based MAC (SELinux on RHEL/Fedora, AppArmor on Debian/Ubuntu/SUSE) and would break drop-in compat REQ-1. **Default-policy refinement (decision 2026-05-09)**: each absorbed feature is **ON by default and configurable** (sysctl/Kconfig/per-process prctl as appropriate per feature). Carve-out: NOEXEC-strict and MPROTECT-W→X-block default OFF at system level because default-on breaks every JIT (V8/OpenJDK/.NET/BEAM/LuaJIT/PyPy → Chrome/Firefox/Java/.NET); per-process opt-in via existing `prctl(PR_SET_MDWE)`. The MPROTECT-default-inversion question (default-on with per-process exemption mechanism) is open in `security/00-overview.md` Q3. **Rookery is grsec-flavored at the memory layer and LSM-respecting at the policy layer, with secure defaults.** Binding spec is the deferred `00-security-principles.md` (issue #2); the full default-policy table lives in `security/00-overview.md` § Locked-in row-1 absorption list. |
